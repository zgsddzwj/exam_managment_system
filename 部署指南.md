...
2. **配置Judge0 API（必须）：**
   - 访问 https://rapidapi.com/ 注册/登录
   - 搜索并订阅 "Judge0 CE" API（免费套餐即可）
   - 在 Dashboard → Security 中获取 API Key
   - 填入 `.env` 文件中的 `JUDGE0_API_KEY`

详细配置请参考 `backend/.env.example` 文件。
...
nano .env.production
# 设置: VITE_API_BASE_URL=https://your-domain.com/api
# 或使用IP: VITE_API_BASE_URL=http://your-server-ip/api
...
```bash
sudo -u postgres psql -d exam_management
```

### 方式4：可视化工具

可以使用 Navicat、DBeaver 等工具连接PostgreSQL数据库。
...
- 确认collectstatic已运行: `python manage.py collectstatic --noinput`
- 检查Nginx配置中的static路径
- 检查文件权限: `chmod -R 755 /var/www/exam_management/backend/staticfiles`

### 问题3: 数据库连接失败

- 检查PostgreSQL服务: `sudo systemctl status postgresql`
- 检查.env配置中的数据库信息
- 检查PostgreSQL认证配置: `/etc/postgresql/*/main/pg_hba.conf`
- 检查防火墙规则

### 问题4: Judge0 API 401错误

- 检查 `.env` 文件中的 `JUDGE0_API_KEY` 是否正确
- 确认已在RapidAPI上订阅Judge0 CE服务
- 检查API Key是否过期
- 重启后端服务: `sudo supervisorctl restart exam_management`

### 问题5: 前端无法连接后端

- 检查前端 `.env.production` 中的 `VITE_API_BASE_URL` 配置
- 确认后端API可以访问: `curl http://localhost:8000/api/`
- 检查Nginx配置中的 `/api/` 代理设置
- 查看浏览器控制台错误信息

### 问题6: Node.js版本过低

如果前端构建失败，可能需要升级Node.js：
...
- Gunicorn日志: `/var/www/exam_management/backend/logs/gunicorn_*.log`
